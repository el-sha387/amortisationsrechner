...
function NumInput({
  ...
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs font-semibold uppercase tracking-wide" style={{ color: "#3D5278" }}>
        {label}
      </label>
...
export default function Calculator() {
  ...
  const ergebnis = useMemo(
    () => berechne(investition, termine, gehalt, raumkosten, mix, annahmen),
    [investition, termine, gehalt, raumkosten, mix, annahmen]
  );
    ...
    <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  style={{ borderColor: stufe === null ? "#3D5278" : "#e5e7eb", fontFamily: "var(--font-body)" }}
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              ...
              <div className="flex justify-between text-sm font-semibold mt-1" style={{ color: "#3D5278", fontFamily: "var(--font-body)" }}>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <MetricCard label="Einnahmen / Monat" value={`${fmt(ergebnis.einnahmen)} €`} sub={`${fmt(ergebnis.umsatzTermin, 2)} € / Termin`} accent="#AADD00" />
              <MetricCard label="Kosten / Monat" value={`${fmt(ergebnis.ausgaben)} €`} sub="inkl. Abschreibung & Gehalt" accent="#e5e7eb" />
              <MetricCard
      ...
      {activeTab === "annahmen" && (
        <div className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              <h2 className="font-bold text-sm uppercase tracking-wide mb-4" style={{ color: "#3D5278", fontFamily: "var(--font-heading)" }}>
                Einnahmen pro Termin
              </h2>
              <div className="space-y-4">
                <NumInput
                  label="Sattel-Analyse (inkl. Sattelverkauf)"
                  value={annahmen.d1Umsatz}
                  onChange={(v) => setA("d1Umsatz", v)}
                  suffix="€ netto"
                  step={0.5}
                  hint="UVP Dienstleistung: 89 € + anteiliger Sattelumsatz"
                />
                <NumInput
                  label="Bikefitting Basis (inkl. Sattelverkauf)"
                  value={annahmen.d2Umsatz}
                  onChange={(v) => setA("d2Umsatz", v)}
                  suffix="€ netto"
                  step={0.5}
                  hint="UVP Dienstleistung: 149 € + anteiliger Sattelumsatz"
                />
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              ...
              <div className="space-y-4">
                <NumInput
                  label="Abschreibungszeitraum"
                  value={annahmen.abschreibungMonate}
                  onChange={(v) => setA("abschreibungMonate", v)}
                  suffix="Monate"
                  step={6}
                  min={12}
                ...
                <NumInput
                  label="Jahreslizenz"
                  value={annahmen.lizenzMonat}
                  onChange={(v) => setA("lizenzMonat", v)}
                  suffix="€ / Monat"
                  step={1}
                />
                <NumInput
                  label="Ersatzfolie (Gesamtkosten)"
                  value={annahmen.ersatzfolieGesamt}
                  onChange={(v) => setA("ersatzfolieGesamt", v)}
                  suffix="€"
                  step={50}
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              ...
              <div className="space-y-4">
                <NumInput
                  label="Raumgröße Messfläche"
                  value={annahmen.raumQm}
                  onChange={(v) => setA("raumQm", v)}
                  suffix="m²"
                  step={1}
                  min={5}
                />
                <NumInput
                  label="ISCO-Kurs / Jahr"
                  value={annahmen.iscoJahr}
                  onChange={(v) => setA("iscoJahr", v)}
                  suffix="€"
                  step={10}
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm">
              ...
              <div className="space-y-4">
                <NumInput
                  label="Arbeitszeit pro Termin"
                  value={annahmen.arbeitszeitTermin}
                  onChange={(v) => setA("arbeitszeitTermin", v)}
                  suffix="Std."
                  step={0.25}
                  min={0.5}
                ...
                />
                <NumInput
                  label="Lohn-Nebenkosten"
                  value={annahmen.lohnNebenkosten}
                  onChange={(v) => setA("lohnNebenkosten", v)}
                  suffix="%"
                  step={1}
                  hint="Aufschlag auf Bruttogehalt"
                />
                <NumInput
                  label="Vollzeit-Stunden / Monat"
                  value={annahmen.vollzeitStunden}
                  onChange={(v) => setA("vollzeitStunden", v)}
                  suffix="Std."
                  step={1}
                  min={100}
                />
          ...
          <button
            onClick={() => setAnnahmen(DEFAULT_ANNAHMEN)}
...
function MetricCard({
  ...
  return (
    ...
    >
      <div className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: highlight ? "white" : "#6b7280", fontFamily: "var(--font-heading)" }}>
        {label}
      </div>
      <div className="font-bold text-xl leading-tight" style={{ color: highlight ? "white" : "#1f2937", fontFamily: "var(--font-body)" }}>
        {value}
      </div>
